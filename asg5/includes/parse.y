...
%token STRING TILDE TRY VBAREQUAL WHILE WITH YIELD INTNUMBER FLOATNUMBER
%type<id> NAME
%type<intNumber> INTNUMBER pick_PLUS_MINUS pick_unop pick_multop augassign star_trailer trailer comp_op
%type<floatNumber> FLOATNUMBER
%type<stmts> suite plus_stmt global_stmt star_COMMA_NAME
...
%type<node> funcdef return_stmt compound_stmt if_stmt flow_stmt stmt simple_stmt fpdef
...
funcdef // Used in: decorated, compound_stmt
	: DEF NAME { SymbolTableManager::getInstance().funcDepth += 1; } parameters COLON suite 
          { 
              if($2 != NULL && $6 != NULL)
              {   
              	  Node* id = new IdentNode($2);
                  Node* func = new FuncNode(id, $6, SymbolTableManager::getInstance().funcDepth);
                  if(SymbolTableManager::getInstance().funcDepth == 0) { $$ = func; }
                  else { $$ = NULL; }
                  const std::string str = static_cast<IdentNode*>(id)->getIdent();
	          SymbolTableManager::getInstance().setFunctionTable(str,func);
                  pool.add(id);
                  pool.add(func);
              }
...
small_stmt // Used in: simple_stmt, star_SEMI_small_stmt
	: expr_stmt    { $$ = $1; }
	| print_stmt   { $$ = new PrintNode($1); pool.add($$);}
	| del_stmt     { $$ = NULL; }
	| pass_stmt    { $$ = NULL; }
	| flow_stmt    { $$ = $1; }
	| import_stmt  { $$ = NULL; }
	| global_stmt  { $$ = new GlobalNode($1); pool.add($$);}
	| exec_stmt    { $$ = NULL; }
	| assert_stmt  { $$ = NULL; }
	;
...
if_stmt // Used in: compound_stmt
	: IF test COLON suite star_ELIF ELSE COLON suite
          { $$ = new IfNode($2, $4, $8); pool.add($$); }
	| IF test COLON suite star_ELIF
          { $$ = new IfNode($2, $4, NULL); pool.add($$); }
	;
